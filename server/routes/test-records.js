    ...
    try {
        ...
        const testRecordResult = await query(
            `INSERT INTO test_records 
             (tester_id, medical_staff_id, institution_id, test_start_time, test_status, maca_score, mmse_score, grip_strength, open_eyes_result_id, closed_eyes_result_id, created_at) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
            [testerId, currentStaff.id, institutionId, testDate, testStatus, macaScore, mmseScore, gripStrength, openEyesResultId, closedEyesResultId]
        );
